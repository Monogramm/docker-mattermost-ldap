...
init() {
    ...
    if [ ! -f "./.docker-version" ]; then
        log "[TODO] Mattermost-LDAP init to $(cat /opt/Mattermost-LDAP/.docker-version)..."
        ...
        log "[TODO] Mattermost-LDAP update from $(cat ./.docker-version) to $(cat /opt/Mattermost-LDAP/.docker-version)..."
        # Install server Oauth
        rsync -r /opt/Mattermost-LDAP/oauth/ /var/www/html/
        # Get config file
        rsync /var/www/html/oauth/config_db.php.example /var/www/html/oauth/config_db.php; rsync /var/www/html/oauth/LDAP/config_ldap.php.example /var/www/html/oauth/LDAP/config_ldap.php
        rm /var/www/html/oauth/config_db.php.example /var/www/html/oauth/LDAP/config_ldap.php.example
    fi
